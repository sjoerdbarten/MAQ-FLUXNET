...
import glob
from get_AAMS_meteo import *

process_year = 2022

#Setting GLOB
directory = 'W:\ESG\DOW_MAQ\MAQ_Archive\MAQ-Observations.nl\data\AD_FLUX\\'
files = sorted(glob.glob(directory + "ADAM_flux*.csv"))

#We need all files from process_year + Jan 01 from next year and Dec 31 from previous year
filtered_files = [
    file for file in files
    if file == f"{directory}ADAM_flux{str(int(process_year)-1)}1231.csv"
    or file.startswith(f"{directory}ADAM_flux{str(int(process_year))}")
    or file == f"{directory}ADAM_flux{str(int(process_year)+1)}0101.csv"
]
...
cols = ['TIMESTAMP_START','TIMESTAMP_END','Tau','qc_Tau','H','qc_H',
        ...
        'ch4_flux','qc_ch4_flux',
        'H_strg','LE_strg','co2_strg','ch4_strg',
        'co2_mixing_ratio','h2o_mole_fraction','ch4_mixing_ratio',
...
df['ch4_flux'] = df['ch4_flux']*1000    #umol m-2 s-1 to nmol m-2 s-1
df['ch4_strg'] = df['ch4_strg']*1000    #umol m-2 s-1 to nmol m-2 s-1
df['ch4_mixing_ratio'] = df['ch4_mixing_ratio']*1000    #ppm to ppb
...
rename_dict = {
    ...
    'h2o_flux': 'FH2O_1_1_1',
    'ch4_flux': 'FCH4_1_1_1',
    'qc_ch4_flux': 'FCH4_SSITC_TEST_1_1_1',    
    'H_strg': 'SH_1_1_1',
    'LE_strg': 'SLE_1_1_1',
    'co2_strg': 'SC_1_1_1',
    'ch4_strg': 'SCH4_1_1_1',
...
meteo_data = get_aams_meteo_func(process_year)
...
for year in merged_df['TIMESTAMP_START'].str[:4].unique():
    if year == str(process_year): #Save only process year
        ...
        output_path = os.path.join(output_dir, f"FLUXNET_AAMS_{year}.csv")
